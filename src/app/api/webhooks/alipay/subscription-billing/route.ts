...
import { parseAlipayRecurringBillingNotification } from "@/features/payment/recurring-provider-notify";
import {
...
export const POST = withApiLogging(async (request: Request) => {
  try {
    const payload = await parseAlipayRecurringBillingNotification(request);

    if (payload.status !== "paid") {
      const result = await markSubscriptionBillingFailed({
        outTradeNo: payload.outTradeNo,
        providerPaymentId: payload.providerPaymentId,
        failureReason: payload.failureReason ?? "alipay billing failed",
